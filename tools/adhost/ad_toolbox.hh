...
struct ToolboxCanvas {
  ...
  void setup(int W,int Hh){
    ...
    // layout: pixelType@0x1E, pixelSize@0x20, cmpCount@0x22, cmpSize@0x24, pmTable@0x2A.
    ...
    // ALSO keep the legacy (mislocated) offsets populated: Confetti Factory reads pmTable at
    // the old 0x1C and derailed when it went to 0. Writing both is redundant but regression-safe.
    mem->write_u16b(g_pm+0x13, 8);                 // legacy pixelSize
    mem->write_u16b(g_pm+0x16, 1); mem->write_u16b(g_pm+0x18, 1); // legacy cmpCount/cmpSize
    mem->write_u32b(g_pm+0x1C, cth);               // legacy pmTable
